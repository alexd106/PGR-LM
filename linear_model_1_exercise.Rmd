...
# of unusually large forest area values (check your data exploration)
# to remind yourself
...
# The Adjusted R-squared value is 0.582 and therefore 58.2% of
...
12\. Have a go at writing a short 'Data analysis' section for your paper/thesis Chapter Materials and Methods and also a 'Results' section. Keep this brief but remember to include all relevant information and summary statistics.
...
# Data Analysis
# Bird abundance was modelled as a function of forest patch area using a linear 
# model. Prior to analysis, patch area was log10-transformed to account for the 
# strongly right-skewed distribution of values driven by a small number of 
# unusually large patches. Model assumptions were assessed via standard 
# residual diagnostics. All analyses were conducted in R (v4.x).

# Results
# There was a significant positive relationship between bird abundance and 
# forest patch area (Figure XX; t_65 = 9.64, p < 0.001), with log10 patch 
# area accounting for 58.2% of the variation in abundance across patches. 
# Bird abundance increased by an estimated 9.78 individuals (SE = 1.02) for 
# each unit increase in log10 patch area.

# or a non p-value centric approach

# Bird abundance increased by an estimated 9.78 individuals (95% CI: 7.79, 11.77) 
# for each unit increase in log10 patch area (Figure XX), with forest patch 
# area accounting for 58.2% of the variation in bird abundance across patches.
```

\ 

13\. OK, this is an optional question so feel free to skip if you've had enough! (you can find the R code for this question in the exercise solutions if you want to refer to it at a later date). Let's recreate the plot you made in Question 11, but this time we'll add the 95% confidence intervals in addition to the fitted values. Remember, you will need to use the `predict()` function again but this time include the`se.fit = TRUE` argument (store these new values in a new object called `pred_vals_se`). When you use the `se.fit = TRUE` argument with the `predict()` function the returned object will have a slightly different structure compared to when you used it before. Use the `str()` function on the `pred_vals_se` to take a look at the structure. See if you can figure out how to access the fitted values and the standard errors. Once you've got your head around this you can now use the `lines()` function three times to add the fitted values (as before) and also the upper and lower 95% confidence intervals. Don't forget, if you want the 95% confidence intervals then you will need to multiply your standard error values by the critical value of 1.96. Actually, strictly speaking, for a linear model you should use the appropriate t critical value rather than 1.96, since 1.96 is the normal approximation. With 65 residual degrees of freedom the t critical value at $\alpha$ = 0.05 is approximately 2.00, so in practice the difference is negligible here. If you need a reminder then take a look at the video on confidence intervals on the course MyAberdeen site (in the 'Introductory statistics lectures' folder which you can find in the 'Course Resources' learning module). 
...
# Since the model was fitted on the log10 scale, presenting the fitted values on 
# the original scale produces a curve rather than a straight line, which is expected. 
# In an applied setting this is often requested as stakeholders and managers will 
# generally find predictions on the original scale more interpretable. However, 
# there are a few important caveats. The model fits less well on the back-transformed 
# scale, particularly for the two unusually large patches and for patches with high 
# bird abundance. The back-transformed curve also passes through a region of the 
# x-axis where there are no intermediate-sized patches, implying a fitted 
# relationship in a part of the data space that is not actually supported by 
# observations. Both of these are expected consequences of the highly skewed 
# distribution of patch sizes and the decision to model on the log10 scale, but 
# they serve as a reminder that back-transformed plots should be interpreted with 
# care and presented cautiously to audiences unfamiliar with the underlying 
# modelling decisions. 

# Despite these caveats the model remains useful, particularly 
# for making predictions within the range of the observed data on the log10 scale. 
# For example, it could be used to predict bird abundance for a patch of a given 
# size, or to estimate the increase in abundance associated with an increase in 
# patch area, both of which are practically relevant questions in a conservation 
# or land management context. Extrapolation beyond the range of the observed data, 
